...
This cacheability in \ac{REST} applications is achieved mainly by idempotence of certain requests.
...
All other stellar objects would not posses such a field and it is left out in the serialized response.
...
It accepts a boolean value as argument, and skips the annotated field or fragment in the response if the argument is \texttt{true}.
...
\autoref{img:graphiql-introspection} shows a screenshot the web-based tool GraphiQL using introspection to programmatically generate a human readable documentation of the \ac{API} as shown on the right-hand side, but also allow developers to write and execute queries against the \ac{API}.
...
\paragraph{Apollo Server and Apollo Gateway}
